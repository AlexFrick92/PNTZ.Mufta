...
Для анализа большой кодовой базы используй slash команды:
- `/analyze-architecture` - анализ архитектуры проекта
- `/analyze-dpworkers` - обзор всех DpWorkers
- `/verify-feature` - проверка реализации функции
- `/check-tests` - анализ тестового покрытия
- `/gemini` - создать custom Gemini команду

Gemini CLI имеет огромное контекстное окно и может проанализировать весь проект целиком.